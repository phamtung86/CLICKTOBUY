...
const ProductMilk = ({ cart, setCart }) => {
  const [dataProductsMilk, setDataProductMilk] = useState([]);
  const navigate = useNavigate();
  useEffect(() => {
    const fetchDataProductMilk = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/api/Products/getDataProductsType?CategoryID=${1}`);
        setDataProductMilk(response.data);
      } catch (error) {
        console.error("Error fetching data:", error);
      }
    };
    fetchDataProductMilk();
  }, []);

  // Sử dụng useMemo để random sản phẩm chỉ một lần
  const randomProducts = useMemo(() => {
    const shuffled = [...dataProductsMilk].sort(() => Math.random() - 0.5);
