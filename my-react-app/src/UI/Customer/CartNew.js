...
const CartNew = () => {
    ...
    const getDataOrder = () => {
        return new Promise((resolve, reject) => {
            ...
            if (!userIDSession) {
                reject("User session is not available");
                return;
            }
    ...
    const getAccountFromSession = async (event) => {
        ...
        try {
            // Đợi dữ liệu đơn hàng từ hàm getDataOrder
            const dataOrder = await getDataOrder();

            // Gửi dữ liệu thanh toán qua API
            const postDataCart = await axios.post("http://localhost:8080/api/Orders/InsertOrders", {
                value: dataOrder,
            });
            ...
            alert("Lỗi khi kết nối tới máy chủ hoặc dữ liệu đơn hàng không hợp lệ. Vui lòng thử lại.");
        }
    };
