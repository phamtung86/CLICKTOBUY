        ...
        try {
            switch (type) {
                ...
                case "MILK":
                    ...
                    await fetchData('http://localhost:8080/api/Products/getDataProductsMilk');
                    break;
                case "VEGETABLE":
                    setValueType("Rau - Củ - Trái cây");
                    await fetchData('http://localhost:8080/api/Products/getDataProductsVegetable');
                    break;
                case "CLEANCHEMICAL":
                    setValueType("Hóa Phẩm - Tẩy Rửa");
                    await fetchData('http://localhost:8080/api/Products/getDataProductsCleanChemical');
                    break;
                case "TAKECARE":
                    setValueType("Chăm sóc cá nhân");
                    await fetchData('http://localhost:8080/api/Products/getDataProductsTakeCare');
                    break;
                default:
                    setDataProduct([]);
                    setValueType('');
